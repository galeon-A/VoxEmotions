...
# Ensure project root is on sys.path regardless of CWD (Streamlit Cloud compatible)
...
with st.sidebar:
    ...
    if use_mic:
        try:
            from src.audio_engine import AudioEngine as AE, PYAUDIO_AVAILABLE
            if not PYAUDIO_AVAILABLE:
                st.warning("⚠️ PyAudio / PortAudio not available in this environment. Using simulation mode.")
                device_idx = None
                use_mic = False
            else:
            ...
            st.info(f"PyAudio not available ({_e}) — try simulation mode.")
            device_idx = None
            use_mic = False
    else:
        device_idx = None
